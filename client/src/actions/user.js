...
import { v4 as uuidv4 } from 'uuid';
 import uploadFile from '../firebase copy/uploadFile';
...
// test by me create image for login
export const createProfile = async (currentUser, createdFields, dispatch) => {
  dispatch({ type: "START_LOADING" });

  const { name, file } = createdFields;
  let body = { name };
  try {
    if (file) {
      const imageName = uuidv4() + "." + file?.name?.split(".")?.pop();
      const photoURL = await uploadFile(file, `profile/${imageName}`);
      body = { ...body, photoURL };
    }
    const result = await fetchData(
      {
